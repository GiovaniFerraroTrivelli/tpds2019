...
public class GestorClientes {
	...
	public static void altaCliente(altaClienteDTO clienteDTO) throws DatoNoEncontradoException, java.sql.SQLIntegrityConstraintViolationException {
		...
		if (clienteDTO.getDireccion().getDepartamento() != null) direccion.setDepartamento(clienteDTO.getDireccion().getDepartamento());
		direccion.setNumero(clienteDTO.getDireccion().getNumero());
		if (clienteDTO.getDireccion().getPiso() != null) direccion.setPiso(clienteDTO.getDireccion().getPiso());
		try {
			direccion.setLocalidad(
					GestorGeografico.getLocalidad(clienteDTO.getDireccion().getIdLocalidad()));
		...
		Documento documento = new Documento(clienteDTO.getDocumento().getTipoDocumento(), clienteDTO.getDocumento().getNroDocumento());
